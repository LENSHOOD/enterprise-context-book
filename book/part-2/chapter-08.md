...
知识图谱是具体的知识实例：规范实体、属性、关系、声明及其证据。RDF 用主语—谓词—宾语三元组表示图；属性图则通常允许节点和边直接携带属性。两者首先是数据模型选择，会影响语义和查询表达，并不只是某个数据库的物理格式；但企业仍应先定义业务概念，再选择合适的表达与存储。[RDF 1.2 Concepts](https://www.w3.org/TR/rdf12-concepts/) 一个企业完全可以用 Markdown 管理术语、用 JSON Schema 校验接口、用关系数据库保存对象、用属性图执行遍历；只要它们遵守同一逻辑模型，就不必为了“语义化”全部迁移到 RDF。
...
为了避免把“企业上下文”缩成业务流程和任务，本书在建模时同时使用两套坐标。第一套是企业工作层次：
...
L1 战略意图
    ↓
L2 业务架构：价值流、能力、产品和业务领域
    ↓
L3 经营与组织：业务单元、责任、决策权和指标
    ↓
L4 应用与数据架构：应用、服务、数据产品、指标和血缘
    ↓
L5 业务运行：流程、任务、对象、规则、事件和状态
    ↓
L6 Agent 执行：主体、委托、证据、记忆、预算、工具和审批
```

第二套是 EA 的架构视图，例如业务、应用、数据和技术视图。两套坐标不能互相替换。战略目标是企业工作层次中的 L1 对象，但它可能通过业务视图的能力关系、数据视图的指标定义和应用视图的数据服务同时被描述。Agent 不是把这些视图拼成一张无限大的图，而是针对能力问题选择一条有预算、有权限、有证据的路径。

建模工作还需要一条横向治理轴，贯穿所有层次和视图：身份、租户、时间、权威性、来源与血缘、责任人、权限、证据等级和生命周期。没有这条横向轴，平台会把战略目标、旧政策、当前指标和某个 Agent 的临时记忆放在同一张图里，却无法说明它们谁更可信、何时有效、谁能看到或修改。
...
GraphRAG 不是图检索的唯一形态。Graphiti 把时间变化、来源和本体放进面向 Agent 的上下文图；另一类“Context Graph”提案还会记录状态差异、主动提醒的相关性和接收主体。[Graphiti](https://github.com/getzep/graphiti) [Context Graphs for Proactive Enterprise Agents](https://arxiv.org/abs/2607.07721) 这些名称目前没有统一的行业定义。本书把它们当成动态实现模式来比较：它们可能增加事件、任务和主体相关性，但也带来误报、过度提醒、传播和权限泄露的评测要求。

开源项目的维护状态也属于架构判断的输入。Microsoft GraphRAG 仓库当前 README 标注项目处于维护模式，主要进行修复和依赖更新。[GraphRAG repository](https://github.com/microsoft/graphrag) 因此可以把 GraphRAG 当作已公开的研究/工程模式继续学习，但不应把它写成一个持续扩张的官方产品路线。

企业图不必完全从文本抽取。更稳妥的顺序是先接入服务目录、代码索引、数据库元数据和组织系统等确定结构，再让模型补充文档中的概念关系和关键声明。这样构建的图具有“确定性骨架 + 推断性语义”，比纯粹从所有文本生成一张图更容易治理。

### 8.10.1 动态 Context Graph：把变化和相关性也建模

近年的 Context Graph 实践没有形成统一标准，但出现了一个清楚的侧重点：除了“有哪些对象、对象怎样相连”，系统还记录事实怎样变化、哪个任务受到影响、哪些主体需要知道，以及为什么现在应该提醒某个人。Graphiti 把时间变化、来源和本体放进 Agent 的上下文图；一篇关于主动式企业 Agent 的预印本则把状态差异检测、相关性排序和说明层组合起来。[Graphiti](https://github.com/getzep/graphiti) [Context Graphs for Proactive Enterprise Agents](https://arxiv.org/abs/2607.07721)

本书不把 Context Graph 当成知识图谱的替代品。它可以理解为在知识图谱上增加事件、有效时间、任务和主体相关性的运行层。主动提醒还要评估误报、打扰成本、遗漏和权限泄露；一条“可能与某项目有关”的边不能自动变成通知，更不能因为 Agent 判断紧急就绕过授权。对 Northstar 来说，事件变更、队列观察和任务记忆可以成为这层的输入，但当前教学实现只演示确定性查询，不实现主动提醒引擎。
...
| Property | 对象有哪些属性、状态和时间字段？ | 可映射为知识对象内容与状态；双时间是本书另行设计的契约，不是属性字段自动提供的能力 |
...
结合 Palantir 的对象和动作概念，本书建议先从一个需要改善的决策开始；以下七个问题是本书的建模推导，不是逐字转述的厂商方法。例如“某工厂产能下降时，哪些订单应该改派”。建模团队随后回答七个具体问题：谁来决定？他需要哪些事实？哪些对象有自己的身份和生命周期？哪些关系决定影响范围？系统用什么规则生成候选方案？哪些变化允许写回业务系统？最后由谁批准和执行？
