...
逻辑模型把概念变成平台契约：稳定标识、实体类型、关系方向、基数、状态、时间、来源、权限和约束。“团队负责服务”在这里必须拆成开发维护、运行值班或业务问责，不能只保留模糊的 `OWNS`。逻辑模型也规定哪些关系是来源声明，哪些是系统推导，哪些需要人工确认。
...
若首要任务是变更影响分析，可以从 `Service`、`Repository`、`API`、`Table`、`Team` 与 `Runbook` 开始，关系只需要 `CALLS`、`IMPLEMENTS`、`READS`、`WRITES`、`OWNS` 和 `DOCUMENTED_BY`。若任务是客户支持，还需加入产品、政策、地区和工单。每增加一种边，都要回答它如何产生、如何更新以及能支持什么查询。
...
模型必须约束实例，而不是只校验自己的 JSON 是否语法正确。Northstar 的契约测试逐条读取领域模型、知识对象和关系边，验证：每种实例边已经声明；主客体的 `entity_type` 与关系契约一致；`evidence_tier` 属于允许等级；每个能力问题所需的关系至少有一条实例。否则“能力问题存在于模型文件中”并不意味着交付数据真的能回答它。
...
`CodeGraph`、`code_review_graph` 和 `understand-anything` 等名称常指向相邻但不同的尝试：前者泛指从代码提取结构关系，第二类把变更、评审意见和影响路径组织为图，第三类强调自动探索仓库并生成理解产物。它们不是一套公认协议。架构设计应拆解其能力：解析器、符号索引、图模式、摘要生成、检索 API 与可视化，而不是把项目名称当作标准组件。
...
逻辑关系目录为每条边规定契约。例如 `IMPLEMENTS(Repository, API)` 必须来自代码索引或显式映射；`CALLS(Service, API)` 可来自编译级索引、网关配置或运行追踪，但查询结果显示来源差异；`ON_CALL_FOR(Team, Service)` 与 `MAINTAINS(Team, Repository)` 取代含义不清的 `OWNS`。`GOVERNED_BY(RefundEvent, Policy)` 必须同时检查政策适用地区和事件发生时间。
...
一次接口字段变更触发查询。系统先定位 API 版本，再沿 `IMPLEMENTS` 找到仓库，沿反向 `CALLS` 找到调用服务，沿 `OWNS` 找到团队，沿 `DOCUMENTED_BY` 找到需要复核的手册。每条路径显示边来源和版本。一个仅由名称匹配得到的潜在消费者被列入“待确认”，不会与编译器确认的调用者混在一起。

这个结果随后成为上下文包的一部分，由智能体生成变更清单，但真正创建工单或发送通知仍需工具权限和人工确认。图谱缩小了影响范围，没有越过行动治理边界。

## 8.14 模型治理：语义也需要发布流程

领域模型不是一次性设计稿。每个类型和关系应有业务负责人和技术维护者；变更通过提案、样例、兼容性分析、验收查询和迁移计划进入新版本。新增可选属性通常可以向后兼容，拆分实体、改变关系方向或重定义 `OWNS` 一类语义则可能破坏已有查询，必须提供映射和弃用窗口。
