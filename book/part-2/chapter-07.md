...
> 本章要回答：如何把重复的阅读与综合编译成可追踪、可更新的 Wiki？
...
这里的 Wiki 不只是让员工手工编辑网页。它是一层由原始来源编译而来的解释性知识：保留稳定导航、术语、系统导览、主题摘要和证据链接，同时随着来源变化增量更新。Andrej Karpathy 提出的 LLM Wiki 设想将 Raw Sources、Schema、Ingest、Query 与 Lint 组成持续循环，核心思想正是把 Wiki 当作需要重建和检查的派生物，而不是一批生成后不再维护的页面。[LLM Wiki](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f)
...
“编译”这个比喻包含四层含义。输入必须可枚举且有版本；输出遵循明确模式；生成物能追踪到输入；来源变化时可以确定哪些输出失效。它与代码编译不同之处在于，自然语言综合具有不确定性，因此还要记录模型、提示、审核和质量状态。
...
没有 Schema 的自动 Wiki 很快会退化成风格各异的散文。页面模式应按对象类型定义，而不是强迫所有主题使用同一个模板。系统页面可以包含职责、边界、接口、依赖、所有者、运行指标、常见故障和相关仓库；仓库页面可以包含目标、构建方式、模块地图、入口符号和变更热点；业务概念页面可以包含定义、同义词、适用范围、规则、指标与负责人。
...
生成并非只有一次提示调用。稳健流程通常先收集目标页面所需的证据包，再生成结构化草稿，随后逐条验证声明是否被证据覆盖，最后渲染成 Markdown。若来源冲突，草稿应保留冲突块；若必需字段无证据，保留明确空缺。生成器不能为了模板完整而发明所有者、接口或结论。
...
代码可以编译和测试，Wiki 也需要类似的最低质量门槛。结构 Lint 检查必需字段、标题层级和模板版本；链接 Lint 检查坏链接、孤页、循环与不存在的实体；证据 Lint 检查无来源声明、不可解析版本和过期引用；语义 Lint 检查术语冲突、重复页面和同一对象的矛盾所有者。
...
因此，企业代码 Wiki 不应止于“对每个文件做摘要”。它应把仓库视为子图：符号、文件、构建目标、接口、所有者、提交和事故互相连接；页面则是这个图在不同层级的阅读视图。Tree-sitter、SCIP 或编译器索引提供确定结构，模型补充职责解释和主题摘要，版本化源码始终保留为最终证据。

代码 Wiki 的质量也不能按页面长度衡量。有效指标包括入口覆盖率、符号引用正确率、跨仓链接可解析率、变更后陈旧时间以及开发者完成真实理解任务所需的跳转次数。

## 7.8 Northstar 的页面编译循环

Northstar 为退款系统定义三种页面：业务能力页、系统页和仓库页。业务能力页由政策、流程与指标生成；系统页由服务目录、接口、运行手册和架构决策生成；仓库页由 README、构建文件、代码图和关键符号生成。三类页面共享对象 ID，却有不同 Schema 和 ACL。

一次代码提交修改了退款重试策略。代码索引首先更新符号和调用关系，影响分析使仓库页的“重试机制”区块失效。系统页因为引用该机制也进入待重建状态；业务能力页没有直接依赖实现，因此保持不变。新页面生成后，Lint 发现运行手册仍描述旧次数，于是发布流程没有悄悄改写手册，而是创建一个由所有者审核的知识差异项。

客服检索只能看到业务能力页和获准的运行步骤；开发者可以继续下钻到系统页、仓库页和源码。两者阅读的是同一事实链的不同投影，而不是两套彼此漂移的知识库。

## 本章小结

Wiki 是对高频理解工作的提前编译。它通过 Schema、来源血缘、增量摄取、声明验证和 Lint，把零散来源变成可导航、可复用的解释层。Wiki 页面必须明确自己是派生知识：保留版本和证据，区分事实、解释与推断，不复制实时状态，也不覆盖规范来源。下一章将把页面之间隐含的关系提升为一等对象，进入知识图谱。
