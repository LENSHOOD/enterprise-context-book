...
在 `examples/enterprise-case/` 执行 `python3 src/tutorial_experiments.py ablation`。脚本固定对象快照和教学时钟，选取 24 道题中的 12 道检索题，分别运行 BM25、离线语义代理和融合，输出逐题证据召回与行为约束是否满足。图题和工具题不混进这张表，离线代理也不冒充嵌入模型。
