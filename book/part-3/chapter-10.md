...
> 本章要回答：如何把分散的知识能力组装成共享对象、版本、权限和证据契约的平台？

前六章分别讨论了 RAG、混合检索、层级、Wiki、图谱和记忆。若把它们作为六个独立产品部署，用户会面对六套对象 ID、六套权限、六种更新时间和彼此矛盾的答案。企业上下文基础设施的任务不是继续增加组件，而是让这些能力共享对象、版本、权限、证据和评测契约，最终为一个具体任务组装出一致材料。

“上下文基础设施”比“企业知识库”多了一层运行含义。知识库保存相对稳定的制度、文档、代码与解释；上下文还要加入当前用户、任务状态、实时业务数据、可用工具和行动约束。它的最终产品不是某个索引，而是一个有边界、可追溯、适合当前智能体使用的上下文包（Context Package，第 17 章给出完整结构）。

## 10.1 九段生命周期

完整平台可以沿九段生命周期理解：来源连接、解析规范化、知识编译、多模型索引、查询规划、混合召回、证据验证、上下文打包、反馈治理。
...
从运维职责看，平台可以分为控制面、数据面和服务面。控制面管理连接器、来源范围、Schema、身份映射、策略、流水线版本、发布和回滚。它回答“应该摄取什么、怎样处理、谁可以用、当前发布是哪一个”。

数据面执行批量和流式工作：抓取来源、解析文件、生成嵌入、抽取代码关系、编译 Wiki、构建快照。它需要幂等、可重试和可观测，因为失败经常来自某个坏文件、限流或模型输出，而不是整条任务。

服务面面向在线请求，负责身份解析、查询规划、检索、上下文包和工具资源。它的核心要求是低延迟、严格权限、可降级和可复现。服务面不能因为向量索引暂时不可用就绕过 ACL，也不能为了低延迟静默使用陈旧且不兼容的图快照。
...
## 10.4 Snapshot Manifest：让一次回答可复现

上下文查询通常跨多个后端。如果 BM25 使用政策 v3、向量库仍是 v2、图中已经出现 v4 的接口，融合结果可能形成现实中从未同时存在的世界。Snapshot Manifest 用一个发布清单绑定各投影的兼容版本。
...
一次回答的审计记录只需保存 Manifest ID、查询计划、候选对象 ID 与最终引用，就能在权限允许时重放。清单也使回滚成为整体操作：发现新嵌入模型导致召回下降时，恢复上一发布，而不必手工猜测哪些索引需要回退。
...
具体产品会变化，但以下契约应长期稳定：对象可定位、版本不可歧义、权限在暴露前执行、来源可回溯、派生物有血缘、快照可复现、失败可降级、行动需独立授权、结果可评测。
...
Northstar 的参考实现将从单体开始：Python 负责摄取与 Context API，PostgreSQL/pgvector 保存对象和向量，轻量图保存关系，Markdown 保存可审阅 Wiki，对象存储保存原始工件。组件数量不是重点；每个对象贯穿这些投影且能够被测试，才是架构成立的证据。

## 本章小结

上下文基础设施沿九段生命周期把来源转化为任务材料，并通过控制面、数据面和服务面分离治理、批处理与在线服务。统一对象让词法、向量、图和 Wiki 成为同一知识的投影，Snapshot Manifest 保证多索引一致和回答可复现，Context API 则把证据、记忆、实时观察和缺口以明确类型交给 Agent。平台的长期资产不是某个数据库，而是一组跨组件保持不变的可信契约。
