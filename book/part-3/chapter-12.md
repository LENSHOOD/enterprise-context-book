...
在目标企业中，客服可以检索退款政策并读取自己租户的订单状态，但不能读取退款服务源码；工程师能查看代码和内部 Runbook，却不能查看其他租户订单；财务主管可以批准大额退款，但 Agent 没有独立批准权。当前 Northstar fixture 实际演示的是 support、developer、sre 和 incident_commander 四个角色，以及队列重放的 `SRE -> incident_commander -> SRE` 交接；财务退款审批属于生产扩展场景。

下面仍是财务退款的目标扩展示例，不能据此推断本仓库已经实现财务接口。当用户要求处理一笔退款时，Agent 先读取订单和政策，生成建议与引用。退款工具返回预览，包括金额、账户、原因和幂等键。主管确认绑定这些参数后，工具才执行，并再次读取交易状态验证结果。任何检索文档中的指令都不能改变审批阈值。
