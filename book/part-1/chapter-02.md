...
企业上下文继承了几十年的知识管理和信息系统实践。文档保存解释，数据库保存结构化事实，搜索提供发现，专家系统约束决策，图谱表达关系，向量跨越措辞，LLM 综合证据，Agent 将知识带入行动。理解这段历史，可以避免把任何新组件误认为完整答案。
