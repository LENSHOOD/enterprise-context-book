...
本章先设计 Agent 运行时的上下文契约和行动控制面，再把它映射到 Northstar 的 C5、C6。代码仍是可运行证明，但不再承担全部解释。
...
第一，`manifest` 固定模型和知识投影，不固定实时世界。即使两次诊断使用同一快照，队列深度也可能不同。第二，`semantic_contract` 会带上本次选择的能力问题及其最小类型、术语和关系契约；检索结果即使混入其他主题，也不能擅自扩大这个任务边界。`competency_coverage` 再明确本次可见证据与路径覆盖了哪些要求、还缺哪些。契约说明“要回答什么”，覆盖报告说明“这次是否拿到了”，二者不能混为一谈。低权限主体即使知道一个公开任务契约，也不会因此得到对象实例、关系路径或来源正文。第三，`missing` 不是补写答案的邀请，而是让规划器知道哪些实时资源尚未取得。第四，`allowed_tools` 是策略结果，不是模型建议；Agent 可以选择是否调用，但不能自行增加能力。
...
可以把边界压缩为一句话：本体定义可理解的业务世界和可允许的变化，Context Package 为一次任务选择其中的可见部分，工作流决定此刻是否可以推进，工具网关最终执行并验证变化。Palantir 将这些能力放在一体化产品中；Northstar 将它们拆成可替换的教学组件，以便读者看清各层责任。
...
Context Package 把身份、知识快照、相关语义契约、稳定证据、实时观察、任务记忆、缺口和工具能力组织为类型化输入，使模型不必从一段长提示词中猜测对象含义与信任边界。行动系统再把建议拆成预览、确认、执行、回执和独立验证，并用状态机约束每一步的主体与前置条件。Northstar 的 C5–C6 只是一套内存教学 fixture，却验证了企业实现不可丢失的契约：模型约束先于关系使用，读权限不等于写权限，批准绑定具体参数，幂等不绕过身份，完成由真实结果定义。
