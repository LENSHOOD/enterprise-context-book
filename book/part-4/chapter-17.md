...
本章先说明一次任务要给 Agent 哪些材料，以及行动中的每一步由谁控制，再把设计对应到 Northstar 的 C5、C6。代码负责证明这些规则可以运行，正文负责解释为什么要这样设计。
...
## 17.14 把设计映射到 Northstar 的 C5-C6
...
Context Package 把身份、知识快照、相关语义规则、稳定证据、实时观察、任务记忆、已知缺口和工具能力分别装好。模型不必再从一段长提示词中猜测每段内容是什么意思、能信到什么程度。行动系统随后把建议拆成预览、确认、执行、回执和独立验证，并用状态机规定每一步由谁完成、必须先满足什么条件。Northstar 的 C5–C6 只是一套内存教学 fixture，却验证了企业实现不可丢失的规则：模型约束先于关系使用，读权限不等于写权限，批准绑定具体参数，幂等不绕过身份，完成由真实结果定义。
