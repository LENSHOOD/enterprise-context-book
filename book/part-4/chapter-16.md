...
> 本章要回答：怎样先让系统正确理解企业里的对象和关系，再生成可以查询的图、可以阅读的 Wiki、任务记忆和架构对照？
...
因此，检索结果只是找到的材料，知识模型才规定这些材料在企业里是什么意思。模型建立后，同一批对象还要以四种方式使用：图让系统沿关系查找，Wiki 帮人快速理解，任务记忆记录这次工作进行到哪里，企业架构声明则说明系统按设计应该是什么样。它们不是五套独立知识库，而是同一套企业语言、对象规则和证据规则生成的不同视图。

本章先完整走过 Northstar 的知识建模过程，再解释四种投影怎样生成和保持一致。C3 的完成标志不是“画出一张好看的概念图”，而是能从人工可评审材料编译出模型，并让错误术语、错误边方向、缺少源映射和无法回答的能力问题在构建时失败；C4 才开始生成图、Wiki、任务记忆和架构对照。C7 的战略快照采用同样的原则，只是它把模型范围上移到企业意图、经营结构和指标语义，并用独立的可比较性检查保护分析入口。

## 16.1 知识对象说明“它是谁”，知识模型说明“它是什么意思”

第 15 章的统一对象规则回答“这份材料是谁、来自哪里、谁能看、何时有效”。知识模型继续回答“它在业务中代表什么、能和哪些对象建立什么关系、这条关系要有什么证据、最终帮助谁做什么决定”。这两层不能合并：一份 Markdown 即使已经带上完整的版本和权限，其中的“服务”“消费者”仍可能只是没有解释清楚的词。
...
阅读本章时可以按四组推进：§16.1–16.6 是建模输入，§16.7 是实例验证，§16.8–16.14 是查询和使用，§16.15–16.16 是变更与动手验证。先完成一组，再进入下一组。

前四类要由业务专家、架构师和平台团队共同评审；最后一类则由程序生成，可以删除后重建。两者必须分开。如果直接手改程序生成的文件，团队讨论确认的业务含义就会再次和系统实际执行的规则分叉。

## 16.2 第一步：从决策写出可验收的能力问题

“建设企业知识图谱”不是能力问题，因为它没有可判定结果。Northstar 先选择四个需要真实决策的问题：

1. C-level 询问 H2 销售为什么低于 H1 时，指标口径、目标、业务分解、组织责任和数据缺口是什么？
2. API 变化时，哪些服务、代码仓、值班团队和 Runbook 必须进入评审？
3. `order.cancelled` 变化时，哪些消费者和测试构成最低影响范围？
4. 一次退款事件发生时，哪一版政策在该地区有效？

每题除了自然语言，还记录所支持的决策、必需对象类型和关系类型。例如第三题不是要求系统“谈论事件影响”，而是要求模型至少能表达 `EventSchema -> CodeSymbol -> Test`，并在 fixture 中找到 `CONSUMED_BY` 和 `TESTED_BY` 的实例。第四题则迫使模型区分“政策发布时间”“事件发生时间”和“系统首次观察时间”。第一题额外要求指标定义、战略目标、业务能力、组织、应用、数据产品和经营观察能够回到同一快照；它不能只靠一张销售报表回答。

能力问题在这里有三个作用。第一，它限制建模范围，避免团队试图一次描述整个企业。第二，它决定先接入哪些数据源，避免先处理与任务无关的资料。第三，它直接成为验收题，避免模型只声明了一堆类型，却没有任何真实对象和关系可以回答问题。战略题还要求把“能算出差异”和“能解释原因”分开：分解事实可以由同一快照核算，因果假设必须有额外证据。新增类型如果不服务任何问题，应先解释未来用途；新增问题如果依赖尚未建模的关系，则应让构建明确失败，而不是让模型临时从文本中猜。
...
模型规定一种关系允许表达什么，实例证据则说明这一条具体关系为什么存在。交换主客体后，边不再符合契约；仅凭名称相似得到的候选，也不能伪装成 `resolved`。

`GOVERNED_BY(RefundEvent, Policy)` 除了限制两端类型，还要求地区匹配，并且**政策有效区间**要覆盖事件发生时间。关系自身的有效区间表达的是另一件事：这条“事件映射到政策”的解析结论，在系统的哪段知识状态中成立。

因此，`many_to_one_at_event_time` 并不表示“一个事件永远只能出现一条政策边”。同一事件可以保留先前映射和后续纠正，但指向两个不同政策的**关系有效区间**不能重叠。这样既能保存修订历史，也不会让同一个解释时刻出现两份互相冲突的适用政策。

不要用事件发生时间检查关系的修订窗口，否则事后纠错会被误判为非法。两条时间线各管一件事：事件发生时间用来选择当时适用的政策版本；关系有效时间用来选择系统在某个时刻采用的解析结论。因此，一条关系不是图数据库里随手连上的线，而是一项有来源证据、可以被纠正、能够按时间解释和重建的业务声明。
...
## 16.7 第六步：生成运行模型，再用真实实例验收

`build_domain_model.py` 完成两层验证。第一层检查模型本身：术语在各自边界内是否冲突，类型是否有身份、边界和术语，关系端点、基数与证据等级是否合法，能力问题是否引用已声明元素，每个模型元素是否有负责人明确的源映射。第二层读取 `compile_knowledge()` 合并的十九个对象与 `relations.json`：对象类型必须已声明，所有声明类型与关系都必须被 fixture 实际使用，边的方向、证据等级与时间信封必须符合关系契约，每道能力问题要求的类型和关系还必须落在同一个连通答案子图中。
...
这里的 `supported` 只证明回答问题所需的类型和关系已经有真实实例。它不证明检索已经找全，也不证明模型最后生成的自然语言答案正确。后续图查询测试验证路径，Golden Questions 验证任务行为，人工评审验证业务定义。模型编译、实例契约、查询评测和答案评测是四个不同质量门，不能用其中一个替代全部。
...
## 16.8 同一套模型，生成四种用途不同的视图

同一个 `order.cancelled` 对象可以被四类使用方式引用；其中任务记忆来自执行事件，架构声明来自设计资产，不能误认为都由文档自动派生：
...
Northstar 的 `TaskMemory` 以“租户 + `task_id`”保存有序事件，并且只有绑定的任务操作者能读取。它足以演示成员隔离和状态恢复语义，但仍是进程内 fixture。生产任务服务还需处理多成员授权、并发写、事件版本、重试、保留期、敏感字段、人工更正与删除。
...
这些差异不是要求系统自动修复，而是进入待处理队列。架构负责人可能更新声明，开发团队可能修正实现，平台团队也可能发现解析器误报。保留声明来源和实现证据，比压成一个“架构一致性得分”更有助于做出正确处置。

这使企业架构从偶尔绘制的静态图，变成持续接受代码和运行反馈的上下文来源；同时也避免用一次不完整扫描否定架构意图。第 8 章讨论的知识建模与 MEAF 等企业架构方法，在这里通过同一对象和关系契约发生连接。

### 16.14.1 战略分析怎样沿企业架构向下追踪

战略问题需要一条比退款处置更长的证据路径：
...
Northstar C7 将这条路径编译成七层 Context Package。`enterprise_profiles` 描述商业模式，`strategic_objectives` 描述经营方向，能力和价值流描述业务架构，业务单元描述责任，应用和数据产品描述信息系统，观察和候选事件描述运行材料，L6 则记录本次主体、快照、缺口和只读权限。各层对象仍然可以从不同 MEAF 视图观察，层次只是帮助查询规划器选择任务所需的材料。

这条路径的终点不是“找到一个最像根因的事件”。如果 EMEA 销售下降和渠道冻结都发生在 H2，平台可以返回一条带证据的候选关联；只有补充客户级订单、时间序列、对照组和责任人复核，才能继续讨论因果。知识模型的价值，是把缺少哪类证据说清楚。
...
## 16.16 把设计映射到 Northstar 的 C3-C4 与 C7
...
# C7：先看待确认的指标契约；确认后读取战略目标到运行事实的完整切片。
...
读者应验证的不是命令是否打印 JSON，而是一条可检查链：能力问题决定所需类型和关系；术语、身份、关系反例和源映射组成可评审模型；模型拒绝错误实例；通过验证的边才能进入图；开发者看到三条消费者路径而 support 无法穿过代码边界；Wiki 的每个输入都有版本化引用；任务记忆按租户和操作者隔离；架构差异不会把“缺少证据”误写成“声明错误”；战略题在口径未确认时不计算，在确认后能把总额、目标差距、维度分解、组织责任和数据血缘放进同一个企业级包。
...
可信对象解决“找到了什么”，知识模型解决“这些对象在企业里是什么意思、可以怎样连接”。图、Wiki、任务记忆和企业架构，则让人和系统用不同方式理解并持续使用它们。Northstar 先写出能力问题，再整理术语、身份规则、关系契约和源映射，随后生成运行模型，并用真实对象和关系证明它确实能回答这些问题。C7 进一步说明企业级范围不能停在流程和任务：同一个上下文平台还要能够从商业模式和战略意图走到经营事实，再把结果压缩成当前主体可使用的只读分析材料。

四种视图各有分工：图把有证据的关系变成有边界的路径；Wiki 把反复发生的阅读整理成按安全域生成的解释页面；任务记忆保存一次工作的进展；生产中可另设审核，把经验证经验晋升为长期知识；企业架构把设计意图与实现、运行事实并列比较。它们通过对象引用与模型版本相连，分别执行自己的权限规则。生产中还需要血缘失效、审核和持久任务服务；当前样例只实现模板重建、关系校验和进程内事件。

下一章将把语义契约和这些材料组装成 Agent 可以直接使用的 Context Package，并讨论怎样从建议安全地走向行动。
