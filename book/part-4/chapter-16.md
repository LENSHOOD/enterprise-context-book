...
能力问题在这里有三个作用。第一，它限制建模范围，避免团队试图一次描述整个企业。第二，它决定先接入哪些数据源，避免先处理与任务无关的资料。第三，它直接成为验收题，避免模型只声明了一堆类型，却没有任何真实对象和关系可以回答问题。新增类型如果不服务任何问题，应先解释未来用途；新增问题如果依赖尚未建模的关系，则应让构建明确失败，而不是让模型临时从文本中猜。
...
可信对象解决“找到了什么”，知识模型解决“这些对象在企业里是什么意思、可以怎样连接”。图、Wiki、任务记忆和企业架构，则让人和系统用不同方式理解并持续使用它们。Northstar 先写出能力问题，再整理术语、身份规则、关系契约和源映射，随后生成运行模型，并用真实对象和关系证明它确实能回答这些问题。

四种视图各有分工：图把有证据的关系变成有边界的路径；Wiki 把反复发生的阅读整理成按安全域生成的解释页面；任务记忆保存一次工作的进展，并通过审核晋升为长期知识；企业架构把设计意图与实现、运行事实并列比较。它们都引用同一对象和模型版本，继承相同权限，并在来源变化时沿血缘一起失效。
