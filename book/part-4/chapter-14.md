...
若你只想理解架构，可以顺序阅读第 15—17 章的设计正文：第 15 章解释来源如何成为可信证据，第 16 章解释证据怎样组织为图、Wiki、记忆和架构反馈，第 17 章解释这些材料怎样进入 Agent 并形成受控行动。每章末尾再把设计映射到 C0—C5 的命令与测试。若你想亲手构建，请按检查点顺序运行，不要先替换向量数据库，也不要先给 Agent 接写工具。每一层都依赖前一层已经通过的对象、权限、版本和引用契约。
